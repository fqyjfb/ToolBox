...
const StorageTab: React.FC<StorageTabProps> = ({ onClearCache, btnLoading, btnText }) => {
  ...
  const refreshIconCacheStats = useCallback(async () => {
    ...
    const [stats, typeStats] = await Promise.all([
      iconCacheService.getStats(),
      iconCacheService.getStatsByType()
    ]);
    setIconCacheStats(stats);
    setGeneralCacheStats(typeStats.general);
    setPluginCacheStats(typeStats.plugin);
    setAppCacheStats(typeStats.app);
    ...
    <SettingCard>
      ...
      <div className="flex items-center justify-between px-4 py-3 settings-section-header">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center px-2.5 py-1 text-xs hover:bg-menu-hover text-content-primary disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isClearingUserData ? (
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-6 gap-2">
          {statItems.map(item => {
            ...
            return (
              <div key={item.key} className="bg-surface-secondary rounded-lg p-2">
      ...
      <div className="">
        <div className="flex items-center justify-between px-4 py-3 settings-section-header">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs hover:bg-menu-hover text-content-primary disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="px-4 py-3">
          <div className="bg-surface-secondary text-content-primary rounded-md px-3 py-2 text-xs font-mono break-all">
            {dbPath || '使用默认路径'}
          </div>
      ...
      <div className="">
        ...
        <div className="p-4">
            ...
            <div className="bg-surface-secondary rounded-lg p-3">
              ...
              <div className="text-lg font-bold text-gray-900 dark:text-white">
                {isRefreshingIconCache ? (
                  ...
                  `${generalCacheStats.count} / ${formatBytes(generalCacheStats.size)}`
                )}
              </div>
            </div>
            <div className="bg-surface-secondary rounded-lg p-3">
              ...
              <div className="text-lg font-bold text-gray-900 dark:text-white">
                {isRefreshingIconCache ? (
                  ...
                  `${pluginCacheStats.count} / ${formatBytes(pluginCacheStats.size)}`
                )}
              </div>
            </div>
            <div className="bg-surface-secondary rounded-lg p-3">
              ...
              <div className="text-lg font-bold text-gray-900 dark:text-white">
                {isRefreshingIconCache ? (
                  ...
                  `${appCacheStats.count} / ${formatBytes(appCacheStats.size)}`
                )}
              </div>
            </div>
          </div>
          ...
          <p className="text-xs text-content-secondary mt-2">
            图标缓存使用浏览器 Cache API 存储，自动保存 7 天。通用缓存用于网址收藏和导航页面，插件缓存支持离线使用，可单独清理。
          </p>
        </div>
      </div>
