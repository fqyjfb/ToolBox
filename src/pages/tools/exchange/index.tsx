...
const ExchangePage: React.FC = () => {
  ...
  const commonRates = useMemo(() => {
    ...
    return result;
  }, [exchangeData, fromCurrency, commonCurrencies]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const filteredRates = useMemo(() => {
    ...
    return filtered;
  }, [exchangeData, fromCurrency, searchCurrency, currencyNames]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="h-full flex flex-col overflow-hidden">
        <div className="flex justify-end items-center mb-3">
          <div className="flex items-center space-x-2">
            <div className="flex items-center space-x-1 text-xs text-content-secondary">
          ...
          <div className="bg-surface-secondary rounded-lg p-3 mb-3 border border-gray-200 dark:border-content">
            ...
            <div className="flex flex-wrap items-end gap-1.5">
              <div className="flex flex-col gap-0.5 min-w-[100px] flex-1">
                <label className="text-xs text-content-secondary font-medium">从</label>
                <Select
                  value={fromCurrency}
                  onChange={setFromCurrency}
                  options={currencyOptions}
                  className="px-2 py-2 border border-content rounded-md bg-surface text-content-primary dark:text-white focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-gray-500 text-xs"
                />
              </div>
              
              <div className="flex flex-col gap-0.5 min-w-[100px] flex-1">
                <label className="text-xs text-content-secondary font-medium">金额</label>
                <input
                  type="number"
                  value={fromAmount}
              ...
              <div className="flex items-center justify-center p-0.5">
                <button
                  ...
                  className="p-1 rounded-full bg-gray-200 dark:bg-menu-hover hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"
                >
                  <ArrowRightLeft className="w-3.5 h-3.5 text-content-primary" />
                </button>
              </div>

              <div className="flex flex-col gap-0.5 min-w-[100px] flex-1">
                <label className="text-xs text-content-secondary font-medium">到</label>
                <Select
                  value={toCurrency}
              ...
              <div className="flex flex-col gap-0.5 min-w-[100px] flex-1">
                <label className="text-xs text-content-secondary font-medium">结果</label>
                <input
                  type="number"
                  value={toAmount}
                  readOnly
                  className="px-2 py-2 border border-content rounded-md bg-surface-secondary text-content-primary dark:text-white focus:outline-none text-xs"
                />
              </div>
            </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-1.5 gap-1.5 flex-shrink-0">
              ...
              <div className="w-full sm:w-56">
                <div className="relative">
                  <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 w-3 h-3 text-content-tertiary" />
                  <input
                    type="text"
                    value={searchCurrency}
                    onChange={(e) => setSearchCurrency(e.target.value)}
                    placeholder="搜索货币..."
                    className="pl-8 pr-2 py-2 w-full border border-content rounded-md bg-surface text-content-primary dark:text-white focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-gray-500 text-xs"
                  />
                </div>
              </div>
            </div>
