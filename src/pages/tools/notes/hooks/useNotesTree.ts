// useNotesTree —— 文件树/视图/加载/错误；跨 hook 依赖由参数注入，避免循环依赖。
// 目录体系只有两类：对话目录（主进程越界门控的 rootPath，即对话路径）与用户配置的固定目录；
// 文件列表只显示固定目录内容，新建 / 导入只在固定目录内生效。
...
export function useNotesTree(deps: UseNotesTreeDeps): UseNotesTreeReturn {
  ...
  // 识别"视图已变、树没跟上"（如固定目录被移除后视图失效 / 回落其它固定目录），由 viewSync 兜底重载。
  ...
  const fullScan = useCallback(
    async (viewPath: string | null) => {
      ...
      const chatBasePath = chatPathRef.current;
      if (chatBasePath) {
  ...
  // 化的视图路径恢复固定目录视图；无视图则列表为空（不扫描对话目录）。
  const init = useCallback(async () => {
    if (initialized) return;
    if (!window.electron?.notes) return;

    // 目录体系只有两类：对话目录 + 用户配置的固定目录。rootPath 取对话路径，仅用于
    // 主进程越界门控与监听范围，不作为文件列表视图、也不作为新建落点。
    // 无对话路径时保持未初始化，等用户选定后由下方 effect 依赖变化重新触发本函数。
    const root = chatPathRef.current;
    if (!root) return;

    try {
      // 文件列表不扫描对话目录，主根设置只用于内部兼容（见下方 sync effect 同步到主进程）
      setHasRootPath(true);
      setRootPathState(root);

      // 视图恢复条件：仍是被固定的目录。固定目录被全部移除 / 取消固定后，
      // 持久化的视图路径即使磁盘仍存在也不得恢复——否则 init 按该目录取树后，组合根的
      // 成员校验又重置视图，viewSync 补扫与 init 后台刷新并发交错，文件列表会
      // 卡在已取消固定的目录。
      // init 闭包取首次渲染值，pinnedFolders 在挂载时即从 localStorage 同步初始化。
      let viewPath =
        deps.currentViewPath && deps.pinnedFolders.some((p) => p.path === deps.currentViewPath)
      ...
      if (memoryTree && memoryRoot === root && memoryView === viewPath) {
        ...
        treeViewRef.current = viewPath;
        setInitialized(true);
        void fullScan(viewPath).catch(() => {
          /* 后台刷新失败保留缓存树，由下次刷新 / 手动刷新自愈 */
        });
        return;
      }
      await fullScan(viewPath);
    } catch {
      setError('初始化笔记模块失败');
    }
    setInitialized(true);
  ...
  // 对话路径变更 → 同步对话目录到状态与主进程门控（rootPath = 对话路径）。
  // 文件列表只显示固定目录，故此处不触发扫描。
  useEffect(() => {
  ...
  const switchToFolder = useCallback(
    async (folderPath: string) => {
      ...
      treeViewRef.current = folderPath;
      const seq = beginScan(folderPath);

      try {
        setLoading(true);
        // 只把视图根同步给门控；主进程主根保持为对话目录，二者互不覆盖
        await notes.setViewPath(folderPath);
        // 切换途中又产生了更新的扫描（快速连切 / 移除固定目录），或视图已再次变化
        // → 让最新的当前视图扫描负责结果，本次旧目录树丢弃
        const tree = await notes.getFileTreeAsync(folderPath);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [beginScan, discardStaleScan, deps.loadChatOrganizeTree, deps.setCurrentViewPath]
  );

  // 重建索引覆盖全部两类目录：对话目录 + 当前查看的固定目录（同一目录只跑一次）
  const rebuildIndex = useCallback(async () => {
    if (!window.electron) return;
    const targets = [
      ...new Set(
        [chatPathRef.current, currentViewPathRef.current].filter((p): p is string => Boolean(p))
      ),
    ];
    ...
    try {
      ...
      for (const target of targets) {
        await window.electron.notes.indexAll(target);
      }
    } catch (err) {
