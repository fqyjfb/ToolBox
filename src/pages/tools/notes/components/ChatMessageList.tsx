...
export const ChatMessageList: React.FC<ChatMessageListProps> = ({
  ...
  return (
    ...
    >
      {messages.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center text-content-tertiary">
          <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-surface-secondary dark:bg-content-primary">
        ...
        <div className="flex flex-col items-center gap-2">
          {Object.entries(messagesByDate).map(([date, dateMessages]) => (
            <div key={date} className="w-full max-w-[720px]">
              <div className="px-2 py-1 text-xs font-medium text-content-tertiary">
                {date}
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`chat-message flex cursor-pointer items-start gap-1.5 rounded px-2 py-1.5 text-xs transition-colors ${
                        isSelected
                          ...
                          : 'hover:bg-surface-secondary/$1 dark:hover:bg-content-primary/$1'
                    ...
                    >
                      ...
                      <button
                        className={`mt-px flex h-4 w-4 flex-shrink-0 items-center justify-center rounded border transition-colors ${
                          message.done
                            ...
                            : 'border-content hover:border-success'
                        }`}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <p className={`min-w-0 flex-1 whitespace-pre-wrap break-words leading-relaxed ${
                        message.done ? 'text-content-tertiary line-through' : 'text-content-primary'
                      }`}>
                        {message.text}
                      </p>
                      ...
                      <div className="relative flex h-5 w-[130px] flex-shrink-0 items-center justify-end">
                        {!showActions && (
                          <span className="absolute inset-y-0 right-0 flex items-center text-xs text-content-tertiary">
                            {message.timestamp}
                          </span>
                        )}
