    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <QrCode className="w-6 h-6 text-content-secondary" />
      ...
      <div className="flex-1 bg-surface rounded-lg shadow-md overflow-auto">
        <div className="p-6 flex flex-col lg:flex-row gap-8 items-start justify-center">
          <div className="flex-shrink-0">
            {qrDataUrl ? (
              <div className="bg-surface-secondary p-4 rounded-lg shadow-md">
              ...
              <div className="w-64 h-64 bg-surface-secondary rounded-lg flex items-center justify-center">
                <QrCode className="w-16 h-16 text-content-tertiary" />
              </div>
            )}
          </div>
          ...
          <div className="flex-1 w-full max-w-md space-y-4">
            <div>
              <label className="block text-sm text-content-secondary mb-2">内容</label>
              <textarea
                value={input}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-content-secondary mb-2">尺寸</label>
                <Select
                  value={String(size)}
                  onChange={(v) => setSize(Number(v))}
                  options={[{ value: '128', label: '128 x 128' }, { value: '256', label: '256 x 256' }, { value: '384', label: '384 x 384' }, { value: '512', label: '512 x 512' }]}
                  className="w-full px-4 py-2.5 border border-content rounded-lg bg-surface text-content-primary outline-none focus:border-blue-500"
                />
              </div>
              
              <div>
                <label className="block text-sm text-content-secondary mb-2">容错级别</label>
                <Select
                  value={level}
            ...
            <div>
              <label className="block text-sm text-content-secondary mb-2">颜色</label>
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block text-xs text-content-secondary mb-1">前景色</label>
                  <div className="relative">
                    <input
                      type="color"
                      value={colorDark}
                      onChange={(e) => setColorDark(e.target.value)}
                      className="w-full h-10 rounded-lg cursor-pointer border border-content"
                    />
                  </div>
                </div>
                <div className="flex-1">
                  <label className="block text-xs text-content-secondary mb-1">背景色</label>
                  <input
                    type="color"
                    value={colorLight}
